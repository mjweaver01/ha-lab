...
- [ ] A **webhook orchestrator** accepts HTTP POST events from a node, persists them, and notifies subscribers.
- [ ] **Clients** can register **subscription endpoints** (e.g. URLs) or in-app channels for events for a home.
- [ ] A **simulated node** (script or small process) sends sample events into the orchestrator without real sensors/cameras.
...
| Webhook orchestrator as single Bun HTTP service | One place to log, route, and replay events for debugging | — Pending |
